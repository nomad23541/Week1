public class Part2 {
	...
	private static String[] customers = new String[] { "Amy Garrison", "Braydon Beil", "Bluebell Nguyen", "Konrad Sears",
	...
	public static void main(String[] args) {
		...
		String format = "%-17s %-8s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %s\n";
		...
		for (int i = 0; i < customers.length; i++) {
			// consolidate each account into an array of tokens
			String[] accountTokens = new String[15];
			accountTokens[0] = customers[i];
			int missedPayments = 0;
			
			for (int j = 0; j < payments[i].length; j++) {
				double value = payments[i][j];
				if(j == 0) {
					accountTokens[1] = String.valueOf(value);
				} else {
					accountTokens[j + 1] = String.valueOf(value);
			...
			accountTokens[accountTokens.length - 1] = standing;
			System.out.printf(format, (Object[]) accountTokens);
		}
	}
