...
public class Part3 {
	...
	private static String[] customers = new String[] { "Amy Garrison", "Braydon Beil", "Bluebell Nguyen",
			"Konrad Sears", "Cheryl Salas", "Rosanna Meyer", "Karis Haley", "Enzo Donaldson", "Catrina Griffiths",
			"Gregor Payne", "Jermaine Jordan", "Zakariya Goulding", "Mehdi Vickers", "Zarah Lozano", "Mattie Watkins",
			...
			"Kady Whitworth", "Sadiyah Levine", "Sakina Cresswell", "Chantel Wu", "Harriette Booth", "Isla-Mae Edge",
			"Daniela Shepherd", "Cosmo Lam", "Myron Archer", "Caspian Bradford", "Elyse Fletcher", "Halima Devlin",
			"Humzah Madden", "Bevan Donnelly", "Piotr Rice", "Malikah Mejia", "Mujtaba Bassett" };

	private static double[][] payments = new double[][] { { 1032, 47, 43, 38, 61, 69, 99, 47, 36, 30, 99, 59, 17 },
	...
	// format to use when printing customer data
	private static String format = "%-17s %-8s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %-5s %s\n";
		...
		switch (menuOption) {
			case 1:
				...
				do {
					...
					account = findByAccountNumber(input.nextInt());
				} while(account == null);
				printReportHeader();
			...
			case 2:
				printReportHeader();
				for(String s : findAccountsWithMissedPayments()) {
					System.out.println(s);
				}
				break;
			case 3:
				printReportHeader();
				for(String s : findClosedAccounts()) {
					System.out.println(s);
				}
				break;
			case 4:
	...
	private static void printReportHeader() {
		System.out.println("----------------------------------------------------------------------------------------------------------");
		System.out.printf(format, "Name", "Account", "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "Standing");
		System.out.println("----------------------------------------------------------------------------------------------------------");
	}
	
	private static String getAccountAtIndex(int index) {
		StringBuilder sb = new StringBuilder();
		
		String customerName = customers[index];
		int missedPayments = 0;
		
		sb.append(String.format("%-18s", customerName));
		
		for(int i = 0 ; i < payments[index].length; i++) {
			double value = payments[index][i];
			// explicit int cast to get correctly formatted account number
			if(i == 0) {
				sb.append(String.format("%-9s", Integer.toString((int) value)));
			} else {
				if(value == 0) missedPayments++;
				sb.append(String.format("%-6s", Double.toString(value)));
			}	
		}
	...
	private static String findByAccountNumber(int accountNumber) {
		for(int i = 0; i < customers.length; i++) {
			int account = (int) payments[i][0];
			if(account == accountNumber) {
				return getAccountAtIndex(i);
			}
		}
	...
	private static List<String> findAccountsWithMissedPayments() {
		List<String> missedPayments = new ArrayList<String>();
		for(int i = 0; i < customers.length; i++) {
			String account = getAccountAtIndex(i);
			if(!account.endsWith("Good")) missedPayments.add(account);
		}
		
		return missedPayments;
	}
	
	private static List<String> findClosedAccounts() {
		List<String> closed = new ArrayList<String>();
		for(int i = 0; i < customers.length; i++) {
			String account = getAccountAtIndex(i);
			if(account.endsWith("Closed")) closed.add(account);
		}
